...
| Fighters | Fighter Freezer: fighter stats, levels, decals, bags and inventories; raise the freezer level (adds hangers), add a new fighter into an empty hanger (the way the Fighter Depot does), delete a fighter the way the game does, and recover a dead fighter (Hater) for free |
